...
export default function MileStoneCard(
    ...
    const markAsComplete = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.preventDefault()
        toggleMilestoneStatus({ variables: { toggleMilestoneCompletedId: _id } })
    }
    const deleteHandler = (e: any) => {
        e.preventDefault()
        deleteCompanyMilestone({ variables: { deleteCompanyMilestoneId: _id } })
    }
    ...
    return (
        <Col md={5}
            ...
            // conditional classnames for border
            className={`shadow-sm p-4 m-3  border border-muted ${milestoneCompleted ? 'border-success' : 'border-muted'}`}
        ...
        >
            <motion.div
                ...
                <div className='d-flex justify-content-between align-items-start my-2'>
                    <Modal isOpen={deleteModal} toggle={toggleDeleteModal} size="lg" >
                        ...
                        <ModalFooter>
                            ...
                            <Button color="danger" outline onClick={(e) => deleteHandler(e)}>
                                yes delete
                            </Button>{' '}
                        </ModalFooter>
                    </Modal>
                ...
                <span className="  text-muted" > due:  {moment(milestoneDueDate).format('MM DD YYYY')} </span>
                <br />
                <small>descriptoin   <br />  </small>
                ...
                <div className="d-flex justify-content-between align-items-center my-3 w-100" >
                    <Button color={milestoneCompleted ? 'warning' : 'success'} size="sm" outline className='px-4'
